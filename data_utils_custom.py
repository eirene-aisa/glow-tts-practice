...
from korean_text import text_to_sequence
from text import cmudict
from korean_text.symbols import kor_symbols # pitchtron
# from korean_text.symbols_korean import kor_symbols # nvidia-tacotron2
...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    def get_text(self, text):
        ...
        if self.add_blank:
            text_norm = commons.intersperse(text_norm, len(kor_symbols)) # add a blank token, whose id number is len(symbols)
...
class TextMelSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_sid_text, hparams):
        ...
        self.max_text_len = getattr(hparams, "max_text_len", 140) # 제한 190 -> 
        if getattr(hparams, "cmudict_path", None) is not None:
          self.cmudict = cmudict.CMUDict(hparams.cmudict_path)
        self.stft = commons.TacotronSTFT(
            hparams.filter_length, hparams.hop_length, hparams.win_length,
